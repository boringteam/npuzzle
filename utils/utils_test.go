...
func TestReturnPossibleMoves(t *testing.T) {
	a := []int8{1, 2, 3, 4, 0, 6, 7, 5, 8}
	InitUtils(a)
	res_up := []int8{1, 0, 3, 4, 2, 6, 7, 5, 8}
	res_down := []int8{1, 2, 3, 4, 5, 6, 7, 0, 8}
	res_left := []int8{1, 2, 3, 0, 4, 6, 7, 5, 8}
	res_right := []int8{1, 2, 3, 4, 6, 0, 7, 5, 8}
	res := [][]int8{res_up, res_down, res_left, res_right}
	if fmt.Sprint(ReturnPossibleMoves(a)) != fmt.Sprint(res) {
		t.Error("Error: ReturnPossibleMoves")
	}
	b := []int8{1, 2, 0, 4, 3, 6, 7, 5, 8}
	InitUtils(b)
	res_down = []int8{1, 2, 6, 4, 3, 0, 7, 5, 8}
	res_left = []int8{1, 0, 2, 4, 3, 6, 7, 5, 8}
	res = [][]int8{res_down, res_left}
	if fmt.Sprint(ReturnPossibleMoves(b)) != fmt.Sprint(res) {
		t.Error("Error: ReturnPossibleMoves")
	}
	c := []int8{1, 2, 3, 4, 7, 6, 0, 5, 8}
	InitUtils(c)
	res_up = []int8{1, 2, 3, 0, 7, 6, 4, 5, 8}
	res_right = []int8{1, 2, 3, 4, 7, 6, 5, 0, 8}
	res = [][]int8{res_up, res_right}
	if fmt.Sprint(ReturnPossibleMoves(c)) != fmt.Sprint(res) {
		t.Error("Error: ReturnPossibleMoves")
	}
	d := []int8{3, 5, 1, 0, 8, 4, 2, 6, 7}
	res_up = []int8{0, 5, 1, 3, 8, 4, 2, 6, 7}
	res_down = []int8{3, 5, 1, 2, 8, 4, 0, 6, 7}
	res_right = []int8{3, 5, 1, 8, 0, 4, 2, 6, 7}
	res = [][]int8{res_up, res_down, res_right}
